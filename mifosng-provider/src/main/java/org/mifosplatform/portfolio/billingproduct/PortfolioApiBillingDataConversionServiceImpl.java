...
Set<String> modifiedParameters = new HashSet<String>();
	...
	@Override
	public EmployeeCommand convertJsonToEmployeeCommand(Object object,
			String jsonRequestBody) {
		if(StringUtils.isBlank(jsonRequestBody)){
			throw new InvalidJsonException();
		}
		...
		Type typeOfMap = new TypeToken<Map<String,String>>(){}.getType();
		Map<String,String> requestMap = gsonConverter.fromJson(jsonRequestBody, typeOfMap);
		Set<String> supportedParams = new HashSet<String>(Arrays.asList("employeeName","employeeDescription","mobileNumber","email","street","city","state","intermediate","degree","doctorate","twoWheel","fourWheel"));
		checkForUnsupportedParameters(requestMap, supportedParams);
		Set<String> modifiedParameters = new HashSet<String>();
		
		String employeeName = extractStringParameter("employeeName",requestMap,modifiedParameters);
		String employeeDescription = extractStringParameter("employeeDescription",requestMap,modifiedParameters);
		Long mobileNumber = extractLongParameter("mobileNumber", requestMap, modifiedParameters);
		String email = extractStringParameter("email", requestMap, modifiedParameters);
		String street = extractStringParameter("street",requestMap,modifiedParameters);
		String city = extractStringParameter("city",requestMap,modifiedParameters);
		String state = extractStringParameter("state",requestMap,modifiedParameters);
		String intermediate = extractStringParameter("intermediate", requestMap, modifiedParameters);
		String degree = extractStringParameter("degree", requestMap, modifiedParameters);
		String doctorate = extractStringParameter("doctorate", requestMap, modifiedParameters);
		String twoWheel = extractStringParameter("twoWheel",requestMap,modifiedParameters);
		String fourWheel = extractStringParameter("fourWheel",requestMap,modifiedParameters);
		
		return new EmployeeCommand(employeeName,employeeDescription,modifiedParameters,mobileNumber,email,street,city,state,intermediate,degree,doctorate,twoWheel,fourWheel);
	}

	@Override
	public InventoryItemDetailsCommand convertJsonToItemDetailsCommand(Object object,
			String jsonRequestBody) {
		...
		Integer grnId = null;
		String provisioningSerialNumber = null;
		Integer itemMasterId = null;
		String remarks = null;
		Set<String> modifiedParameters = null;
		String quality = null;
		String status = null;
		...
		Type typeOfMap = new TypeToken<Map<String,String>>(){}.getType();
		Map<String,String> requestMap = gsonConverter.fromJson(jsonRequestBody, typeOfMap);
		Set<String> supportedParams = new HashSet<String>(Arrays.asList("locale","itemMasterId","serialNumber","grnId","provisioningSerialNumber","quality","status","warranty","remarks"));
		checkForUnsupportedParameters(requestMap, supportedParams);
		modifiedParameters = new HashSet<String>();
		itemMasterId = extractIntegerParameter("itemMasterId", requestMap, modifiedParameters);
		serialNumber = extractStringParameter("serialNumber", requestMap, modifiedParameters);
		grnId = extractIntegerParameter("grnId", requestMap, modifiedParameters);
		provisioningSerialNumber = extractStringParameter("provisioningSerialNumber", requestMap, modifiedParameters);
		quality = extractStringParameter("quality", requestMap, modifiedParameters);
		status = extractStringParameter("status", requestMap, modifiedParameters);
		warranty = extractIntegerParameter("warranty", requestMap, modifiedParameters);
		remarks = extractStringParameter("remarks", requestMap, modifiedParameters);
		//copy validation.msg.invalid.integer.format to properties file
		if(warranty==null || itemMasterId==null || grnId==null){
			List<ApiParameterError> dataValidationErrors = new ArrayList<ApiParameterError>();
			DataValidatorBuilder baseDataValidator = new DataValidatorBuilder(dataValidationErrors).resource("ItemDetails");
			baseDataValidator.reset().parameter("itemMasterId").value(itemMasterId).longGreaterThanZero().notNull();
			baseDataValidator.reset().parameter("warranty").value(warranty).notNull();
			baseDataValidator.reset().parameter("grnId").value(grnId).longGreaterThanZero().notNull();
			
			if (!dataValidationErrors.isEmpty()) {
				throw new PlatformApiDataValidationException("validation.msg.validation.errors.exist", "Validation errors exist.", dataValidationErrors);
			}
		}
		return new InventoryItemDetailsCommand(itemMasterId.longValue(),serialNumber,grnId.longValue(),provisioningSerialNumber,quality,status,warranty.longValue(),remarks,modifiedParameters);
